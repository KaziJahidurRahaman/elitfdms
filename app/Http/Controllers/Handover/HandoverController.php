...
class HandoverController extends Controller {
	...
	public function masterSheet(Request $request)
	{
		...
		if(count(array_unique($sellers)) > 1){
			...
			$fileName =  str_replace(' ', '', $sellers[0]).'_'.time().'.'. 'pdf' ;
			//Log::info('File='. $path . '/' . $fileName);
			$pdf->save($path . '/' . $fileName);
	
			$downloadpath = asset('/mastersheets'.'/'.$fileName);
			Log::info($downloadpath);

			return json_encode(['success' => true, 'data' => $downloadpath]);
			//Log::info('Downloadbel Path='.$downloadpath);
			//return response()->$pdf;
			//return $pdf;
